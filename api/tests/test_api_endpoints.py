...
async def test_activity_detail_ok(client):
    ...
    with (
        ...
        patch("src.main.get_activity_detail", AsyncMock(return_value=mock)),
    ):
...
async def test_rpe_valid_returns_ok(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.set_activity_rpe", AsyncMock(return_value=True)),
    ):
...
async def test_rpe_not_found_returns_404(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.set_activity_rpe", AsyncMock(return_value=False)),
    ):
...
async def test_api_daily_returns_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_daily_summaries", AsyncMock(return_value=[])),
    ):
...
async def test_api_sleep_returns_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_sleep_sessions", AsyncMock(return_value=[])),
    ):
...
async def test_api_hrv_returns_data(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_latest_hrv", AsyncMock(return_value=None)),
    ):
...
async def test_api_hrv_trend_returns_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_hrv_trend", AsyncMock(return_value=[])),
    ):
...
async def test_api_training_status(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_latest_training_status", AsyncMock(return_value={})),
    ):
...
async def test_api_weekly_returns_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_weekly_stats", AsyncMock(return_value=[])),
    ):
...
async def test_api_energy_returns_data(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_energy_metrics", AsyncMock(return_value={})),
    ):
...
async def test_api_training_load_returns_data(client):
    ...
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_training_load_inputs", AsyncMock(return_value=[])),
        patch("src.main.get_activity_hrmax", AsyncMock(return_value=None)),
        patch("src.main.get_user_sex", AsyncMock(return_value=None)),
        patch("src.main.build_training_load", MagicMock(return_value=mock_load)),
    ):
...
async def test_api_ml_history_returns_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_ml_history", AsyncMock(return_value=[])),
    ):
...
async def test_sync_linked_returns_requested(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER_GARMIN)),
        patch("src.main.request_sync", AsyncMock(return_value=None)),
    ):
        r = await client.post("/api/sync")
    assert r.status_code == 200
...
async def test_api_sync_status_returns_data(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_sync_status", AsyncMock(return_value={})),
    ):
...
async def test_profile_update_epilepsy_mode(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.update_epilepsy_mode", AsyncMock(return_value=None)),
    ):
...
async def test_garmin_link_invalid_credentials_returns_400(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.GarminClient", side_effect=Exception("auth failed")),
    ):
        r = await client.post(
            "/garmin/link",
            data={
...
async def test_garmin_unlink_redirects(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER_GARMIN)),
        patch("src.main.set_garmin_unlinked", AsyncMock(return_value=None)),
    ):
...
async def test_seizures_list(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_seizures", AsyncMock(return_value=[])),
    ):
...
async def test_seizure_risk(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_seizure_risk", AsyncMock(return_value={})),
    ):
...
async def test_glucose_default_returns_200(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_glucose_recent", AsyncMock(return_value=[])),
    ):
...
async def test_glucose_stats_returns_200(client):
    with (
        patch("src.main.require_user", AsyncMock(return_value=TEST_USER)),
        patch("src.main.get_glucose_stats", AsyncMock(return_value={})),
    ):
