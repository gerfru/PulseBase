...
function renderAnomaly(today, history) {
    ...
    document.getElementById('anomaly-card').insertAdjacentHTML(
        'beforeend',
        infoBox(
...
function renderReadiness(today, history) {
    ...
    document.getElementById('rf-stats').innerHTML = [
        statTile(
            'Prognose morgen',
            score != null ? `<span class="badge ${cls}" style="font-size:1.2rem">${Math.round(score)}</span>` : '—',
            ciSub,
        ),
        statTile(
            'Ø letzte 30T',
            avg != null ? `<span class="badge ${avgCls}" style="font-size:1.2rem">${avg}</span>` : '—',
            'Readiness 0–100',
        ),
    ...
    if (mmeta?.importances && Object.keys(mmeta.importances).length) {
        ...
        const trainedAt = mmeta.trained_at ? new Date(mmeta.trained_at).toLocaleDateString('de-AT') : '—';
    ...
    document.getElementById('rf-card').insertAdjacentHTML(
        'beforeend',
        infoBox(
...
function renderCorrelations(today) {
    ...
    document.getElementById('corr-card').insertAdjacentHTML(
        'beforeend',
        infoBox(
