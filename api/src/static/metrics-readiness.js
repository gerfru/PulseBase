...
export const READINESS_METRICS = {
    readiness: {
        ...
        render([today, energy, history]) {
            ...
            const autonDetail =
                auton?.deviation != null
                    ? `${auton.deviation >= 0 ? '+' : ''}${auton.deviation.toFixed(2)} σ`
                    : a != null
                      ? ''
                      : 'keine Daten';
            const cogDetail =
                cog?.debt_hours != null ? `${cog.debt_hours.toFixed(1)}h Schulden` : c != null ? '' : 'keine Daten';
            ...
            return {
                        ...
                        ? [
                              {
                                  ...
                                  datasets: [
                                      ...
                                      {
                                          label: 'Autonom',
                                          data: dates.map((d) => autonMap[d] ?? null),
                                          borderColor: C.amber,
                                      ...
                                      {
                                          label: 'Kognitiv',
                                          data: dates.map((d) => cogMap[d] ?? null),
                ...
                formula: [
                    [
                        'Autonome Energie (60%)',
                        'Score = 70 + z × 15  — Anker: z = 0 → 70 (Baseline = gut), ±1σ → 85/55 (SWC)',
                    ],
        ...
        render(data) {
            ...
            return {
                ...
                formula: [
                    ...
                    [
                        'Aus Literatur',
                        'Plews DL et al. (2013) HRV in Elite Athletes · Stanley J et al. (2015) HRV Recovery Speed',
                    ],
                    ['Heuristisch', 'Lookback 60d · TRIMP-Threshold ×1.5 · 7-Tage HRV-Fenster'],
                ],
                science:
                    'Plews et al. (2013) zeigten in 8 Elite-Ausdauersportlern: die HRV-Recovery-Speed nach intensivem Training (TRIMP > 100) ist ein stärkerer Prädiktor für Overtraining-Syndrom als absolute HRV-Werte. Die Hypothese: ein schnell erholter HRV (>+2ms/d in 7d post-training) deutet auf gute parasympathische Plastizität. Ein verzögerter oder negativer Recovery-Slope kann auf zunehmende autonome Maladaptation hindeuten (Stanley et al. 2015). Die TRIMP-Threshold-Strategie (>1.5× Ø) filtert alltägliche Aktivität und identifiziert signifikante Trainings-Reize.',
                sources: [
                    {
                        label: 'Plews DL et al. (2013): HRV Recovery Following Interval Training — IJSPP 8(6)',
                        url: 'https://pubmed.ncbi.nlm.nih.gov/23539253/',
                    },
                    {
                        label: 'Stanley J et al. (2015): Cardiac Parasympathetic Reactivation — Sports Medicine 45(9)',
                        url: 'https://pubmed.ncbi.nlm.nih.gov/26055047/',
                    },
                    {
                        label: 'Buchheit M et al. (2010): Parasympathetic Reactivation in Team Sports — Int J Sports Physiol',
                        url: 'https://pubmed.ncbi.nlm.nih.gov/21152053/',
                    },
                ],
    ...
    recovery: {
        ...
        render(data) {
            ...
            const corrStr =
                corr?.r != null ? `r = ${corr.r.toFixed(2)} (${corr.interpretation}, n=${corr.n})` : 'Zu wenig Daten';
            ...
            return {
                ...
                charts: [
                    {
                        ...
                        datasets: [
                            {
                                label: 'Letzte Nacht',
                                data: data.hrv.map((d) => d.hrv_last_night ?? null),
                                borderColor: C.green,
                                backgroundColor: 'transparent',
                                tension: 0.3,
                                pointRadius: 0,
                    ...
                    },
                    {
                        ...
                        datasets: [
                            {
                                label: 'Schlaf-Score',
                                data: sleepSorted.map((d) => d.sleep_score ?? null),
                                borderColor: C.violet,
                ...
                sources: [
                    ...
                    {
                        label: 'Besedovsky et al. (2019): Sleep and Immune Function — Pflügers Archiv',
                        url: 'https://pubmed.ncbi.nlm.nih.gov/30689000/',
                    },
                    {
