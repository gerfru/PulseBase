...
function buildHealthCharts(daily, labels, sleep, hrvTrend, mlHistory) {
    if (daily.some((d) => d.steps)) {
        ...
        makeChart('steps-chart', 'bar', labels, [
            { label: 'Schritte', data: daily.map((d) => d.steps || 0), backgroundColor: C.indigo, borderRadius: 4 },
        ]);
    } else {
        showEmpty('steps');
    }
    ...
    if (bbDays.length) {
        ...
        makeChart(
            'battery-chart',
            'line',
            bbDays.map((d) => fmtDate(d.date)),
            [
                {
                    label: 'Hoch',
                    data: bbDays.map((d) => d.body_battery_high),
                    borderColor: C.green,
                ...
                {
                    label: 'Niedrig',
                    data: bbDays.map((d) => d.body_battery_low),
    ...
    if (customSleepHist.length >= 2) {
        ...
        makeChart(
            ...
            [
                {
                    label: 'Score',
                    data: customSleepHist.map((s) => s.value),
        ...
        makeChart(
            ...
            [
                {
                    label: 'Score',
                    data: sleepSorted.map((s) => s.sleep_score ?? null),
                    borderColor: C.violet,
                    backgroundColor: 'transparent',
                    tension: 0.3,
                    pointRadius: 0,
    ...
    if (sleepSorted.some((s) => s.deep_sleep_seconds)) {
        ...
        makeChart(
            ...
            [
                {
                    label: 'Tief',
                    data: sleepSorted.map((s) => secToH(s.deep_sleep_seconds)),
                    backgroundColor: C.sleepDeep,
                    stack: 'sleep',
                    borderRadius: 2,
                },
                {
                    label: 'REM',
                    data: sleepSorted.map((s) => secToH(s.rem_sleep_seconds)),
                    backgroundColor: C.sleepRem,
                    stack: 'sleep',
                    borderRadius: 2,
                },
                {
                    label: 'Leicht',
                    data: sleepSorted.map((s) => secToH(s.light_sleep_seconds)),
                    backgroundColor: C.sleepLight,
                    stack: 'sleep',
                    borderRadius: 2,
                },
                {
    ...
    if (customIntHist.length >= 2) {
        ...
        makeChart(
            ...
            [
                {
                    label: 'Moderat',
                    data: customIntHist.map((s) => s.moderate_minutes || 0),
                ...
                {
                    label: 'Intensiv',
                    data: customIntHist.map((s) => s.vigorous_minutes || 0),
        ...
        makeChart(
            ...
            [
                {
                    label: 'Moderat',
                    data: daily.map((d) => d.intensity_moderate || 0),
                    backgroundColor: C.green,
                    stack: 'intensity',
                    borderRadius: 2,
                },
                {
                    label: 'Intensiv',
                    data: daily.map((d) => d.intensity_vigorous || 0),
                    backgroundColor: C.blue,
                    stack: 'intensity',
                    borderRadius: 2,
                },
    ...
    if (daily.some((d) => d.calories_total)) {
        ...
        makeChart('calories-chart', 'bar', labels, [
            {
                label: 'Kalorien',
                data: daily.map((d) => d.calories_total || 0),
